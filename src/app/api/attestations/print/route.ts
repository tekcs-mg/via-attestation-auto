...
async function generateAttestationHTML(attestation: AttestationAuto): Promise<string> {
    ...
    // Générer le QR code en tant qu'image Data URL sur le serveur
    const qrCodeDataUrl = await QRCode.toDataURL(verificationUrl, { width: 88 });

    // Helper pour les labels bilingues
    ...
    return `
      <div style="width: 1120px; height: 400px; border: 1px solid black; display: flex; font-family: sans-serif; font-size: 10px; background-color: white; padding: 4px; box-sizing: border-box; page-break-after: always;">
        
        <!-- Colonne gauche -->
        <div style="width: 23%; border-right: 1px dashed black; padding: 4px 8px; display: flex; flex-direction: column;">
        ...
        <!-- Colonne centrale -->
        <div style="width: 57%; padding: 4px 8px; display: flex; flex-direction: column;">
            ...
            <div style="border: 1px solid black; padding: 8px; margin-top: 4px; font-size: 9px; display: flex; flex-direction: column; gap: 8px;">
                <div style="display: flex; justify-content: space-between;">
                    <div style="display: flex; align-items: center; gap: 8px; width: 100%;">
                        ...
                        <div style="font-weight: bold; color: black; flex-grow: 1; text-align: center; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${attestation.numeroPolice}</div>
                    </div>
                ...
                <div style="display: flex; align-items: center; gap: 8px;">${bilingualLabel("Mpaka fiantohana", "Souscripteur (Nom et Prénoms)")}<div style="font-weight: bold; color: black; flex-grow: 1; text-align: center; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${attestation.souscripteur}</div></div>
                <div style="display: flex; align-items: center; gap: 8px;">${bilingualLabel("Fonenana", "Adresse")}<div style="font-weight: bold; color: black; flex-grow: 1; text-align: center; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${attestation.adresse}</div></div>
                <div style="display: flex; justify-content: space-between; align-items: flex-end; padding-top: 4px;">
                    <div style="display: flex; align-items: flex-end; gap: 8px; flex-grow: 1;">${bilingualLabel("Manan-kery", "Valable du")}<div style="font-weight: bold; color: black; flex-grow: 1; text-align: center; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${format(new Date(attestation.dateEffet), 'dd MMMM yyyy', { locale: fr })}</div></div>
                    <div style="display: flex; align-items: flex-end; gap: 8px; flex-grow: 1; margin-left: 8px;">${bilingualLabel("ka hatramin'ny", "au")}<div style="font-weight: bold; color: black; flex-grow: 1; text-align: center; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${format(new Date(attestation.dateEcheance), 'dd MMMM yyyy', { locale: fr })}</div></div>
                </div>
            </div>
        ...
        <!-- Colonne droite -->
        <div style="width: 20%; border-left: 1px dashed black; display: flex; flex-direction: column;">
            <div style="height: 50%; padding: 8px; display: flex; flex-direction: column;">
                <div style="font-size: 10px; text-align: left;">
                    ...
                    <div style="margin-top: 8px; display: grid; grid-template-columns: auto 1fr; align-items: center; gap: 8px 8px;">
                        <span style="font-weight: bold; color: #1e3a8a;">MARQUE :</span><div style="font-weight: bold; color: black; text-align: right; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${attestation.marque}</div>
                        <span style="font-weight: bold; color: #1e3a8a;">N° IMM :</span><div style="font-weight: bold; color: black; text-align: right; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${attestation.immatriculation}</div>
                        <span style="font-weight: bold; color: #1e3a8a;">VALIDITE DU :</span><div style="font-weight: bold; color: black; text-align: right; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${format(new Date(attestation.dateEffet), 'dd/MM/yyyy')}</div>
                        <span style="font-weight: bold; color: #1e3a8a;">AU :</span><div style="font-weight: bold; color: black; text-align: right; border-bottom: 2px solid #7dd3fc; padding-bottom: 4px;">${format(new Date(attestation.dateEcheance), 'dd/MM/yyyy')}</div>
                    </div>
                </div>
                <div style="font-size: 9px; text-align: center; margin-top: auto; padding: 4px;">
...
export async function GET(request: Request) {
    ...
    try {
        ...
        // Générer une chaîne HTML contenant toutes les attestations
        const htmlPromises = attestations.map(att => generateAttestationHTML(att));
        const fullHtml = (await Promise.all(htmlPromises)).join('');
        
        const htmlWrapper = `
            <html>
                <head><style>@media print { body { -webkit-print-color-adjust: exact; } @page { size: A4 landscape; margin: 0; } }</style></head>
                <body>${fullHtml}</body>
            </html>`;
